defmodule MyLibrary.LibraryTest do
  use MyLibrary.DataCase

  alias MyLibrary.Library

  describe "books" do
    alias MyLibrary.Library.Book

    import MyLibrary.LibraryFixtures
  ...
  describe "loans" do
    ...
    test "create_loan/1 with valid data creates a loan" do
      valid_attrs = %{borrowed_at: ~N[2025-07-15 02:26:00], due_at: ~N[2025-07-15 02:26:00], returned_at: ~N[2025-07-15 02:26:00]}

      assert {:ok, %Loan{} = loan} = Library.create_loan(valid_attrs)
      assert loan.borrowed_at == ~N[2025-07-15 02:26:00]
      assert loan.due_at == ~N[2025-07-15 02:26:00]
      assert loan.returned_at == ~N[2025-07-15 02:26:00]
    end
    ...
    test "update_loan/2 with valid data updates the loan" do
      loan = loan_fixture()
      update_attrs = %{borrowed_at: ~N[2025-07-16 02:26:00], due_at: ~N[2025-07-16 02:26:00], returned_at: ~N[2025-07-16 02:26:00]}

      assert {:ok, %Loan{} = loan} = Library.update_loan(loan, update_attrs)
      assert loan.borrowed_at == ~N[2025-07-16 02:26:00]
      assert loan.due_at == ~N[2025-07-16 02:26:00]
      assert loan.returned_at == ~N[2025-07-16 02:26:00]
    end
